...
## v2.1.2
- Fix the Region information update issue about Region merge [#1377](https://pd/pull/1377)

## v2.1.1
- Fix the issue that some configuration items cannot be set to `0` in the configuration file [#1334](https://pd/pull/1334) 
- Check the undefined configuration when starting PD [#1362](https://pd/pull/1362)
- Avoid transferring the leader to a newly created peer, to optimize the possible delay [#1339](https://pd/pull/1339)
- Fix the issue that `RaftCluster` cannot stop caused by deadlock [#1370](https://pd/pull/1370) 

## v2.1.0
+ Optimize availability
    - Introduce the version control mechanism and support rolling update of the cluster compatibly 
    - [Enable `Raft PreVote`](https://pd/blob/5c7b18cf3af91098f07cf46df0b59fbf8c7c5462/conf/config.toml#L22) among PD nodes to avoid leader reelection when network recovers after network isolation
...
+ Optimize the scheduler
    ...
    - Improve [PD simulator](https://pd/tree/release-2.1/tools/pd-simulator) to simulate the scheduling scenarios
...
## v2.1.0-rc.5
- Fix the issues related to `pd-ctl` reading the Region key [#1298](https://pd/pull/1298) [#1299](https://pd/pull/1299) [#1308](https://pd/pull/1308)
- Fix the issue that the `regions/check` API returns the wrong result [#1311](https://pd/pull/1311)
- Fix the issue that PD cannot restart join after a PD join failure [#1279](https://pd/pull/1279)
- Fix the issue that `watch leader` might lose events in some cases [#1317](https://pd/pull/1317)

## v2.1.0-rc.4
- Fix the issue that the tombstone TiKV is not removed from Grafana [#1261](https://pd/pull/1261)
- Fix the data race issue when grpc-go configures the status [#1265](https://pd/pull/1265)
- Fix the issue that the PD server gets stuck caused by etcd startup failure [#1267](https://pd/pull/1267)
- Fix the issue that data race might occur during leader switching [#1273](https://pd/pull/1273)
- Fix the issue that extra warning logs might be output when TiKV becomes tombstone [#1280](https://pd/pull/1273)

## v2.1.0-rc.3
### New feature
- Add the API to get the Region list by size in reverse order [#1254](https://pd/pull/1254)
### Improvement
- Return more detailed information in the Region API [#1252](https://pd/pull/1252)
### Bug fix
- Fix the issue that `adjacent-region-scheduler` might lead to a crash after PD switches the leader [#1250](https://pd/pull/1250)
