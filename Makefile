...
BASIC_TEST_PKGS := $(filter-out pd/pkg/integration_test,$(TEST_PKGS))

PACKAGES := go list ./...
PACKAGE_DIRECTORIES := $(PACKAGES) | sed 's|pd/||'
...
ifeq ("$(TRAVIS_COVERAGE)", "1")
	@$(GOFAIL_ENABLE)
	CGO_ENABLED=1 ./scripts/retool do $(OVERALLS) -project=pd -covermode=count -ignore='.git,vendor' -- -coverpkg=./... || { $(GOFAIL_DISABLE); exit 1; }
