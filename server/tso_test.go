...
func (s *testTimeFallBackSuite) SetUpSuite(c *C) {
	gofail.Enable("pd/server/fallBackSync", `return(true)`)
	gofail.Enable("pd/server/fallBackUpadte", `return(true)`)
	s.svr, s.cleanup = mustRunTestServer(c)
	s.client = s.svr.client
	mustWaitLeader(c, []*Server{s.svr})
	s.grpcPDClient = mustNewGrpcClient(c, s.svr.GetAddr())
	s.svr.Close()
	gofail.Disable("pd/server/fallBackSync")
	gofail.Disable("pd/server/fallBackUpdate")
